...
if __name__ == "__main__":
    ...
    # Cargar las tasas actualizadas
    tasas = cargar_tasas("data/tasas.json")
    
    # Mostrar tasas actualizadas en pantalla
    print("Tasas actualizadas:")
    for moneda, valor in tasas["USD"].items():
        print(f"USD -> {moneda}: {valor:.4f}")
    print("Última actualización:", tasas["actualizacion"])
    
    # Convertir un precio de ejemplo
    precio_usd = 100.00
    precio_eur = convertir(precio_usd, "EUR", tasas)
    print(f"\n{precio_usd} USD = {precio_eur:.2f} EUR")
    
    # Registrar la transacción
    registrar_transaccion("Laptop", precio_eur, "EUR", "logs/historial.txt")
    print("Transacción registrada en logs/historial.txt")
